...
import be.ugent.intec.ddecap.dna.ImmutableDnaWithBlsVectorByte
...
class OldBinaryPipedRDD[T: ClassTag](
  ...
  extends RDD[Iterator[(Long, ImmutableDnaWithBlsVectorByte)]](prev) with be.ugent.intec.ddecap.Logging {
  ...
  override def compute(split: Partition, context: TaskContext): Iterator[Iterator[(Long, ImmutableDnaWithBlsVectorByte)]] = {

    if(logLevel != null && logLevel != Level.ERROR) {
        Logger.getLogger("be.ugent.intec.ddecap").setLevel(logLevel)
        Logger.getLogger("org.apache.spark.OldBinaryPipedRDD").setLevel(logLevel)
    }
...
//          new OutputStreamWriter(proc.getOutputStream, Codec.defaultCharsetCodec.name), bufferSize))
    ...
    buf.flip()
    List(new Iterator[(Long, ImmutableDnaWithBlsVectorByte)] {
      def next(): (Long, ImmutableDnaWithBlsVectorByte) = {
        if (!hasNext()) {
        ...
        (ByteBuffer.wrap(grp).getLong(), ImmutableDnaWithBlsVectorByte(ByteBuffer.wrap(wrd).getLong(), blsvec))
      }
      def hasNext(): Boolean = {
