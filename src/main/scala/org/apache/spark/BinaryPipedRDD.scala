...
class BinaryPipedRDD[T: ClassTag](
  ...
  extends RDD[(ImmutableDnaPair, Byte)](prev) with be.ugent.intec.ddecap.Logging {
  ...
  override def compute(split: Partition, context: TaskContext): Iterator[(ImmutableDnaPair, Byte)] = {
...
//          new OutputStreamWriter(proc.getOutputStream, Codec.defaultCharsetCodec.name), bufferSize))
    ...
    buf.flip()
    new Iterator[(ImmutableDnaPair, Byte)] {
