...
class CountedBinaryPipedRDD[T: ClassTag](
    prev: RDD[T],
    command: Seq[String],
    procName: String,
    maxMotifLen: Int,
  ...
  extends RDD[(Long, ImmutableDnaWithBlsVector)](prev) with be.ugent.intec.ddecap.Logging {
  ...
  override def compute(split: Partition, context: TaskContext): Iterator[(ImmutableDna, ImmutableDnaWithBlsVector)] = {
...
//          new OutputStreamWriter(proc.getOutputStream, Codec.defaultCharsetCodec.name), bufferSize))
    ...
    val blscounttypesize = 2; // short = 2, char = 1
    ...
    buf.flip()
    new Iterator[(ImmutableDna, ImmutableDnaWithBlsVector)] {
      var counted = 0;
      def next(): (ImmutableDna, ImmutableDnaWithBlsVector) = {
        if (!hasNext()) {
    ...
    //    }
        ...
        // (ByteBuffer.wrap(grp).getLong(), ImmutableDnaWithBlsVector(ByteBuffer.wrap(wrd).getLong(),  getBlsVectorFromCharList(blsvecdata, blsvec)))
        (ByteBuffer.wrap(grp).getLong(), ImmutableDnaWithBlsVector(ByteBuffer.wrap(wrd).getLong(),  getBlsVectorFromShortList(blsvecdata, blsvec)))
      }
      def hasNext(): Boolean = {
