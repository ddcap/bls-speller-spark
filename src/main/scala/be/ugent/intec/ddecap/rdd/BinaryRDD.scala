...
class BinaryRDDFunctions(rdd: RDD[Array[Byte]]) extends Serializable with Logging {
  ...
  def binSizePerPartition(): Array[Long] = {
    rdd.map(x => x.length.toLong).mapPartitions(it => List(it.sum).iterator).collect
  }

  def binSize(): Long = {
    rdd.map(x => x.length.toLong).mapPartitions(it => List(it.sum).iterator).collect.sum
  }
  ...
  def saveAsBinaryFile(path: String): Long = {
    rdd.mapPartitionsWithIndex((idx, it) => {
      ...
      debug("saveAsBinFile() outputfile: " + towrite.length)
      bos.write(towrite)
